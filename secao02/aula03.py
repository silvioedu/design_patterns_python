...
curso1: Curso = Curso()
curso2: Curso = Curso('Padrão de projetos em Python')
curso3: Curso = Curso('Orquestração de containers com Kubernetes', 23)
...
def gerar_fibonacci(qtd: int) -> None:
  if qtd <= 0:
    ...
    print(f'A sequencia fibonacci para a {qtd} termo(s) é: ')
    contador: int = 0
    aux1: int = 0
    aux2: int = 1
    while contador < qtd:
